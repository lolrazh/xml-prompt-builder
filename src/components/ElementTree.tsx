...
const ElementTree: React.FC<ElementTreeProps> = ({
  ...
  return (
    <div className={cn("space-y-2", depth > 0 && "ml-6 pl-2 border-l border-gray-200 dark:border-gray-700")}>
      {elements.map((element) => (
        <div key={element.id} className="space-y-2">
          <div 
            className={cn(
              "flex items-center gap-2 p-2 rounded cursor-pointer group",
              selectedElementId === element.id ? "bg-primary/20" : "hover:bg-gray-100 dark:hover:bg-gray-800"
            )}
          >
